function uk = linear_fcn(Ymin,xk,UTOTmax,A,B,C,Np)
...
% Constraints
c = repmat([ones(1,2),zeros(1,nx+nu-2)],1,Np)';

Al = zeros(Np,(nx+nu)*Np);
for i = 1: Np
    Al(i,(nx+nu)*i-(nx+nu-1):(nx+nu)*i) = [ones(1,2),zeros(1,nx+nu-2)];
end
bl = UTOTmax;
...
coder.extrinsic('linprog'); % may be changed if you want another solver

[Uopt]=linprog(c,Al,bl,Aeq,beq,lb,ub); % 
